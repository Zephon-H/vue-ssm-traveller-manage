  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column class-name="status-col" label="状态" align="center">
        <template slot-scope="scope">
          <el-tag :type="scope.row.productStatus | statusFilter">{{ scope.row.productStatus==1?'开启':'未开启' }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column label="操作" align="center" width="200">
        <template slot-scope="scope">
          <router-link :to="'/base/product/detail/'+scope.row.id">
            <el-button
              size="mini"
              type="primary">详情
            </el-button>
          </router-link>
          <router-link :to="'/base/product/edit/'+scope.row.id">
            <el-button
              size="mini"
              type="primary">编辑
            </el-button>
...
<script>
import { addProduct, getPageList } from '@/api/product'
